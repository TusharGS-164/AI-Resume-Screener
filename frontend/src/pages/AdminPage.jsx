...
function StatCard({ icon: Icon, label, value, color, delay }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</div>
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '2rem 2.5rem', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div className="animate-up" style={{ marginBottom: '2rem' }}>
        <p style={{ fontSize: 12, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Administration</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: '1.25rem' }}>
        ...
        <div className="animate-up" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '1.5rem', animationDelay: '0.25s' }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '1.25rem' }}>Sessions per user</div>
          {chartData.length === 0 ? (
        ...
        <div className="animate-up" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', animationDelay: '0.30s' }}>
          <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>All users</div>
            <span style={{ fontSize: 11, color: 'var(--text-3)', background: 'var(--bg-3)', padding: '2px 10px', borderRadius: 99, border: '1px solid var(--border)' }}>{users.length}</span>
          </div>
          <div>
            {/* Column headers */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px 80px', padding: '8px 1.5rem', borderBottom: '1px solid var(--border)' }}>
              {['User', 'Email', 'Role', 'Sessions'].map(h => (
                <div key={h} style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{h}</div>
              ))}
            </div>
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--bg-4)', border: '1px solid var(--border-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, color: 'var(--gold)', flexShrink: 0 }}>
                        {initials}
                      </div>
                      <span style={{ fontWeight: 500, fontSize: 13, color: 'var(--text)' }}>{u.name}</span>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: 8 }}>{u.email}</div>
                    <div>
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 99, background: u.is_admin ? 'var(--amber-bg)' : 'var(--bg-4)', color: u.is_admin ? 'var(--amber)' : 'var(--text-3)', fontWeight: 500, border: `1px solid ${u.is_admin ? 'var(--amber-border)' : 'var(--border)'}`, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        {u.is_admin ? 'Admin' : 'Member'}
                      </span>
                    </div>
                    <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, color: u.session_count > 0 ? 'var(--gold)' : 'var(--text-3)' }}>
                      {u.session_count}
                    </div>
                  </div>
                )
